...
> **⚠️ Partly superseded.** The click-path below is still accurate for the parts that need
> your logins (plan, payments, domain, theme). But the storefront copy it describes is from
> the abandoned *Deadpan Goods* direction — "We are very serious about stupid things", the
> Uncle Starter Pack — and none of that is the current brand.
>
> **Current storefront:** [`site/index.html`](../site/index.html), generated by
> [`site/build_storefront.py`](../site/build_storefront.py).
> **Products, policies and pages:** [`store/upload_to_shopify.py`](upload_to_shopify.py)
> does all of it in one command; skip step 4 below entirely.

Exact click-path. Everything here needs **your** logins, so this is the part that can't be
automated — but nothing below requires guesswork.
...
⚠️ That section list and its copy are from the old direction. The current storefront —
hero, catalog, the departmental requisition block, brand story and FAQ — is generated into
[`site/index.html`](../site/index.html). Lift the copy from there, not from here.

## Step 4 · Add products — *skip this, it's automated*

Don't add products by hand. One command creates all 16 with images, variants and prices,
plus the four policies and both pages:

```bash
python3 store/upload_to_shopify.py --all --execute
```

See [`upload_to_shopify.py`](upload_to_shopify.py) for the token setup. The manual
click-path that used to be here took about 20 minutes per product.
